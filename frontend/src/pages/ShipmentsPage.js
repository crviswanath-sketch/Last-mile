...
const ShipmentsPage = () => {
  ...
  return (
    <div data-testid="shipments-page">
      ...
      {/* All modals are defined below - they are quite long */}
            ...
            <form onSubmit={handleCreateShipment} className="p-6 space-y-4">
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button type="button" onClick={() => setFormData({...formData, shipment_type: "delivery", pickup_subtype: ""})} className={\`p-4 rounded-lg border-2 transition-all \${formData.shipment_type === 'delivery' ? 'border-primary bg-primary/5' : 'border-slate-200 hover:border-slate-300'}\`} data-testid="shipment-type-delivery">
                  ...
                  <button type="button" onClick={() => setFormData({...formData, shipment_type: "pickup", pickup_subtype: "pickup"})} className={\`p-4 rounded-lg border-2 transition-all \${formData.shipment_type === 'pickup' ? 'border-primary bg-primary/5' : 'border-slate-200 hover:border-slate-300'}\`} data-testid="shipment-type-pickup">
              ...
              {formData.shipment_type === 'pickup' && (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button type="button" onClick={() => setFormData({...formData, pickup_subtype: "pickup"})} className={\`p-3 rounded-lg border-2 transition-all \${formData.pickup_subtype === 'pickup' ? 'border-purple-500 bg-purple-50' : 'border-slate-200 hover:border-slate-300'}\`} data-testid="pickup-subtype-pickup">
                      <p className={\`text-sm font-medium \${formData.pickup_subtype === 'pickup' ? 'text-purple-700' : 'text-slate-600'}\`}>Regular Pickup</p>
                    </button>
                    <button type="button" onClick={() => setFormData({...formData, pickup_subtype: "customer_return"})} className={\`p-3 rounded-lg border-2 transition-all \${formData.pickup_subtype === 'customer_return' ? 'border-purple-500 bg-purple-50' : 'border-slate-200 hover:border-slate-300'}\`} data-testid="pickup-subtype-return">
              ...
              {drivers.filter(d => d.status === 'active').length === 0 ? (
                ...
                <div className="space-y-2">
                  {drivers.filter(d => d.status === 'active').map((driver) => (
                    <button key={driver.id} onClick={() => handleAssignDriver(driver.id)} className="w-full flex items-center justify-between p-4 bg-slate-50 rounded-lg hover:bg-slate-100 transition-all" data-testid={\`select-driver-\${driver.id}\`}>
                      <div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleDeliveryProof} className="p-6 space-y-4">
              ...
              <button type="button" onClick={getCurrentLocation} disabled={gettingLocation} className="w-full h-10 border border-primary text-primary rounded-md hover:bg-primary/5 transition-all flex items-center justify-center gap-2" data-testid="get-location-btn">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button type="submit" disabled={!deliveryProof.image_base64} className="flex-1 h-10 bg-green-600 text-white rounded-md hover:bg-green-700 transition-all disabled:opacity-50" data-testid="submit-delivery-proof-btn">
              ...
              {selectedShipment?.driver_id && (
                <div className="border-t border-slate-200 pt-4">
                  <div className="flex items-center gap-3 mb-3">
                    <input type="checkbox" id="reassign_driver" checked={rescheduleData.reassign_driver} onChange={(e) => setRescheduleData({...rescheduleData, reassign_driver: e.target.checked, new_driver_id: ""})} className="w-4 h-4 rounded border-slate-300 text-primary focus:ring-primary" data-testid="reassign-driver-checkbox" />
                  ...
                  {rescheduleData.reassign_driver && (
                    <div className="space-y-2">
                      <button type="button" onClick={() => setRescheduleData({...rescheduleData, new_driver_id: ""})} className={\`w-full p-3 rounded-lg border-2 text-left transition-all \${!rescheduleData.new_driver_id ? 'border-orange-500 bg-orange-50' : 'border-slate-200 hover:border-slate-300'}\`} data-testid="unassign-driver-option">
                      ...
                      {drivers.filter(d => d.status === 'active' && d.id !== selectedShipment?.driver_id).map((driver) => (
                        <button key={driver.id} type="button" onClick={() => setRescheduleData({...rescheduleData, new_driver_id: driver.id})} className={\`w-full p-3 rounded-lg border-2 text-left transition-all \${rescheduleData.new_driver_id === driver.id ? 'border-primary bg-primary/5' : 'border-slate-200 hover:border-slate-300'}\`} data-testid={\`reassign-driver-\${driver.id}\`}>
                          <p className={\`text-sm font-medium \${rescheduleData.new_driver_id === driver.id ? 'text-primary' : 'text-slate-900'}\`}>{driver.name}</p>
                          <p className="text-xs text-slate-500">{driver.vehicle_number}</p>
                        </button>
                      ))}
                    </div>
                  )}
      ...
      {showUploadModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-slate-200 flex justify-between items-center sticky top-0 bg-white">
              <h3 className="text-lg font-semibold text-slate-900" style={{ fontFamily: 'Barlow Condensed, sans-serif' }}>BULK UPLOAD</h3>
            ...
            <div className="p-6 space-y-6">
              <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <button onClick={handleDownloadTemplate} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-all text-sm" data-testid="download-template-btn">
              ...
              <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                ...
                <input type="file" ref={fileInputRef} accept=".csv" onChange={handleFileUpload} className="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-primary file:text-white hover:file:bg-primary/90" data-testid="upload-csv-input" />
              ...
              {uploadResult && (
                <div className={\`p-4 rounded-lg border \${uploadResult.error_count > 0 ? 'bg-yellow-50 border-yellow-200' : 'bg-green-50 border-green-200'}\`}>
                  ...
                  {uploadResult.error_count > 0 && (
                    <p className="text-sm text-red-700">✗ {uploadResult.error_count} rows failed</p>
                  )}
                </div>
              )}

              <button onClick={() => setShowUploadModal(false)} className="w-full h-10 border border-slate-300 text-slate-700 rounded-md hover:bg-slate-50 transition-all">Close</button>
            </div>
          </div>
        </div>
      )}
